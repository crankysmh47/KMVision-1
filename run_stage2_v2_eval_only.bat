...
set LOG=logs\stage2_v2_eval_%date:~-4,4%%date:~-10,2%%date:~-7,2%_%time:~0,2%%time:~3,2%%time:~6,2%.log
set LOG=%LOG: =0%
mkdir logs 2>nul

echo [%date% %time%] Stage 2 v2 eval-only started >> "%LOG%"
...
echo [%date% %time%] Eval complete. Results: evaluation\results\stage2_v2_holdout\ >> "%LOG%"
echo Done. See %LOG%
