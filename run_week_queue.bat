...
REM --- Step 0: compress labels ---

echo === STEP 0: compress_labels ===

python scripts\compress_labels.py ^

  --input-dir "C:\sem4\KMVision-1 Data\dataset\train_1\labels" ^

  --output-dir "C:\sem4\KMVision-1 Data\dataset\labels_compressed" ^

  >> "%LOGDIR%\00_compress_labels.log" 2>&1
...
python train_phase_c.py ^
  ...
  --init_checkpoint %INIT_PHASE_B% ^

  >> "%LOGDIR%\01_train_run1.log" 2>&1
...
python eval_inference.py ^
  ...
  --checkpoint %RUN1_OUT%/final ^

  --category km ^

  --max-samples 12 ^

  --output-dir %EVAL1_DIR% ^

  >> "%LOGDIR%\01_eval_run1.log" 2>&1
...
python train_phase_c.py ^
  ...
  --init_checkpoint %RUN1_OUT%/final ^

  --use_chatml ^

  >> "%LOGDIR%\02_train_run2.log" 2>&1
...
python eval_inference.py ^
  ...
  --checkpoint %RUN2_OUT%/final ^

  --category km ^

  --max-samples 12 ^

  --output-dir %EVAL2_DIR% ^

  >> "%LOGDIR%\02_eval_run2.log" 2>&1

if errorlevel 1 goto :failed



python scripts\check_eval_gate.py --stage run2 --results-dir %EVAL2_DIR% ^

  --previous-summary %EVAL1_DIR%\latest_summary.json >> "%LOGDIR%\02_eval_gate.log" 2>&1
...
python eval_inference.py ^
  ...
  --checkpoint %RUN3_OUT%/final ^

  --category km ^

  --max-samples 12 ^

  --output-dir %EVAL3_DIR% ^

  >> "%LOGDIR%\03_eval_run3.log" 2>&1

if errorlevel 1 goto :failed



python scripts\check_eval_gate.py --stage run3 --results-dir %EVAL3_DIR% ^

  --previous-summary %EVAL2_DIR%\latest_summary.json >> "%LOGDIR%\03_eval_gate.log" 2>&1
